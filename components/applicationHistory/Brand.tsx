  ...
  return (
    <div class="border m-5 rounded ">
      ...
      {props.status == 1
        ...
        : null}
      {props.status == 2 ? <BrownButton contents="再度申請する" height={10} /> : null}
      {props.status == 0 || props.status == 1 || props.status == 3
        ? <WhiteButton2 contents={statusList[props.status].button} height={6} />
        : null}
    </div>
  );
}
